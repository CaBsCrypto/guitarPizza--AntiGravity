    ...
    async function jc() {
        if (!No) {
            const { createPublicClient: t, http: e, defineChain: s } = await jr(async ()=>{
                const { createPublicClient: n, http: r, defineChain: i } = await import("./index-D-jbv_gB.js");
                return {
...
`);
    ...
    async function Mm({ hash: t, signature: e }) {
        const s = Vr(t) ? t : ga(t), { secp256k1: n } = await jr(async ()=>{
            const { secp256k1: o } = await import("./secp256k1-DTIastov.js");
            return {
                secp256k1: o
            };
...
`;
        ...
        async initialize(e) {
            if (this.initializeProjectSettings(e), this.initControllers(e), await this.initChainAdapters(), this.sendInitializeEvent(e), x.state.enableReconnect ? (await this.syncExistingConnection(), await this.syncAdapterConnections()) : await this.unSyncExistingConnection(), this.remoteFeatures = await GN.fetchRemoteFeatures(e), x.setRemoteFeatures(this.remoteFeatures), this.remoteFeatures.onramp && ha.setOnrampProviders(this.remoteFeatures.onramp), (x.state.remoteFeatures?.email || Array.isArray(x.state.remoteFeatures?.socials) && x.state.remoteFeatures?.socials.length > 0) && await this.checkAllowedOrigins(), x.state.features?.reownAuthentication || x.state.remoteFeatures?.reownAuthentication) {
                const { ReownAuthentication: s } = await jr(async ()=>{
                    const { ReownAuthentication: r } = await import("./features-BGi_4wNw.js");
                    return {
    ...
    class Lp extends YN {
        ...
        async injectModalUi() {
            if (!du && J.isClient()) {
                if (await jr(()=>import("./basic-XQFkG_LU.js"), __vite__mapDeps([7,8,1,2,9])), await jr(()=>import("./w3m-modal-DPewFfWm.js"), __vite__mapDeps([10,8,1,2])), !document.querySelector("w3m-modal")) {
