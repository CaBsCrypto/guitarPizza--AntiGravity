...
window.initGuitarPizza = function (canvasElement, userAddress, onComplete, songUrl, t, options) {
    ...
    let notes = [];
    const latency = (options && typeof options.latency === 'number') ? (options.latency / 1000) : 0;
    ...
    function pollGamepads() {
        ...
        for (let g = 0; g < gamepads.length; g++) {
            ...
            // Lane 0: Button 2 (X) or Button 14 (D-pad Left)
            if ((gp.buttons[2] && gp.buttons[2].pressed) || (gp.buttons[14] && gp.buttons[14].pressed)) {
                currentPressed[0] = true;
            }
            // Lane 1: Button 0 (A) or Button 13 (D-pad Down)
            if ((gp.buttons[0] && gp.buttons[0].pressed) || (gp.buttons[13] && gp.buttons[13].pressed)) {
                currentPressed[1] = true;
            }
            // Lane 2: Button 3 (Y) or Button 12 (D-pad Up)
            if ((gp.buttons[3] && gp.buttons[3].pressed) || (gp.buttons[12] && gp.buttons[12].pressed)) {
                currentPressed[2] = true;
            }
            // Lane 3: Button 1 (B) or Button 15 (D-pad Right)
            if ((gp.buttons[1] && gp.buttons[1].pressed) || (gp.buttons[15] && gp.buttons[15].pressed)) {
    ...
    function update(dt) {
        ...
        for (let i = notes.length - 1; i >= 0; i--) {
            let n = notes[i];
            if (n.isChartNote) {
                n.y = HIT_Y - (n.chartTime + n.latencySec - gameTimer) * moveSpeed;
